...
public class SulfurRecoveryRecipeSerializer extends IERecipeSerializer<SulfurRecoveryRecipe>{
	...
	public SulfurRecoveryRecipe readFromJson(ResourceLocation id, JsonObject json){
		FluidStack output = ApiUtils.jsonDeserializeFluidStack(JSONUtils.getJsonObject(json, "resultfluid"));
		FluidTagInput inputFluid0 = FluidTagInput.deserialize(JSONUtils.getJsonObject(json, "inputfluid0"));
		FluidTagInput inputFluid1 = FluidTagInput.deserialize(JSONUtils.getJsonObject(json, "inputfluid1"));
		...
		return new SulfurRecoveryRecipe(id, output, outputItem, inputFluid0, inputFluid1, 1.0F, energy, time);
	}
